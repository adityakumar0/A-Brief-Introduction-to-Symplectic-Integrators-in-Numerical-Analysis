%Velocity Verlet Method as Symplectic Integrator 2nd order for S.H.O
...
for nt = 2:Nt
   h = hs(nt-1);
   q(:,nt) = q(:,nt-1) + h*p(:,nt-1) + (1/2)*feval(dpdt, q(:,nt-1))*h^2;
   p(:,nt) = p(:,nt-1) + (1/2) * (feval(dpdt,q(:,nt-1))+feval(dpdt,q(:,nt)))*h;
end
q = q.';
p = p.';
